...
const RsaTool: React.FC = () => {
    ...
    return (
        <ToolCard title="RSA 加密/解密 & 签名" description="RSA 非对称加密算法。支持密钥对生成、公钥加密、私钥解密以及数字签名。">
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem', minHeight: 0 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    ...
                    <textarea value={signature} onChange={e => setSignature(e.target.value)} style={{ height: '60px', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', padding: '10px', borderRadius: '8px', fontSize: '0.75rem', fontFamily: 'monospace', resize: 'none' }} placeholder="签名/验签时使用的签名值" />
                </div>

                {/* Editors */}
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', minHeight: 0 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)' }}>待处理数据</span>
